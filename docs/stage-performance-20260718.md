...
剩余主要瓶颈不是内存撮合，而是每个Raft应用批次要向大量per-asset WAL文件写入，并让20个
Raft replica在同一Docker虚拟盘上竞争fsync。达到500万持久化TPS不能靠继续增加同盘consumer：
应把同group副本分散到独立本地NVMe故障域、增加独立Raft group，并进一步评估用分片WAL替代
热路径中的per-asset重复WAL（迁移日志异步派生），同时保留Raft日志重放和结果Outbox恢复语义。
