...
## 再次逐阶段复测（全新 stage2 数据，2026-07-18）

使用新的`trade-core-stage2` Compose project和全新Kafka、Raft、MySQL数据卷。原开发集群停止，
避免资源竞争。输入为1,000,000条命令、32个HTTP长连接、500条/请求、10,000个资产；每一步只
启用目标consumer，并等待完成计数及lag归零。

| 步骤 | 数据量 | 耗时 | 实测吞吐 | 延迟/完整性 |
|---|---:|---:|---:|---|
| HTTP → 命令Kafka | 1,000,000 commands | 676.21ms | **1,478,839 cmd/s** | 0错误；batch p50 8.87ms、p95 17.75ms、p99 78.87ms |
| 命令Kafka → 5副本Raft WAL → 内存撮合 → durable Outbox | 1,000,000 commands | 2.62s | **381,679 cmd/s** | 109个Raft批次；平均25.94ms/批、max 74.58ms；4组apply lag=0 |
| 命令Kafka → order-id分库分表MySQL | 1,000,000 commands | 30.46s | **32,830 cmd/s** | 3,190个事务，约313条/批；commit p50 31.02ms、p90 73.64ms、p99 448.26ms |
| durable Outbox → 结果Kafka | 2,000,000 events | 41.277s Kafka时间戳窗口 | **48,453 events/s** | 16个partition各125,000；高频metrics轮询下的保守结果 |
| 结果Kafka → order-id分库分表MySQL | 2,000,000 events | 62.74s | **31,878 events/s** | 2,015个批量提交，约993 events/批；p50 90.52ms、p90 226.82ms、p99 386.91ms |
| 纯内存撮合（4个独立节点） | 4,000,000 commands | 670.90ms | **5,962,118 cmd/s** | 2,684,069次真实成交；单节点约1,490,530 cmd/s |

最终一致性：命令Kafka→MySQL和命令Kafka→撮合的consumer lag均为0，DLQ与背压拒绝均为0；
10个MySQL分片各有100,000条命令去重记录和200,000条结果事件去重记录，分布完全均匀。结果
Kafka的16个partition各125,000条，也完全均匀。本轮HTTP负载按`instrument = i % 10000`且买卖
方向按`i`奇偶生成，因此同一资产只有同一方向，不产生trade，但每条命令产生Accepted和Resting
两个结果事件；纯内存基准另用85%新单+15%撤单的交叉价格流验证真实成交路径。

本机单阶段结论：入口Kafka约147.9万cmd/s，强持久化撮合约38.2万cmd/s，结果Kafka约4.85万
events/s，而两条MySQL链路约3.2万/s。因此当前完整系统的持续吞吐首先受Docker Desktop上的
MySQL批量提交限制，其次是结果Kafka发布；纯内存撮合已经超过500万TPS。生产扩展顺序应为：
MySQL分片分散到独立NVMe主机并扩大并行consumer，再增加结果Kafka publisher/group，最后按独立
故障域增加Raft group；不能用增加同一虚拟盘上的线程数替代水平扩展。

### 结果落库按“每个DB批量”优化复测

原`TC_ORDER_BATCH_SIZE=1000`是全局Kafka消费批次。按order_id均匀分到10个DB后，每个DB每事务
实际只有约100条，200万事件产生2,015个全局批次、约20,150个DB事务。现新增：

- `TC_EXECUTION_MYSQL_BATCH_PER_DB`：每个物理DB的事务目标和硬上限，默认500。
- `TC_EXECUTION_MYSQL_BATCH_LINGER_MS`：结果消费聚合等待，默认10ms。

consumer总聚合目标自动计算为`DB_COUNT × BATCH_PER_DB`。例如10个DB、每库500时，每次从Kafka
聚合约5,000个事件，按order_id路由后每库约500条；DB worker即使合并多个job，也按配置硬切分，
不会生成超过500条目标更新的事务。尾批或分布短时波动允许小于上限。

在同一200万结果topic上，每轮清空隔离库的结果去重表并把consumer offset重置到0：

| 每DB事务上限 | 全局聚合目标 | 实测全局批次 | 耗时 | 吞吐 | commit p50 / p90 / p99 |
|---:|---:|---:|---:|---:|---:|
| 原实现（约100/DB，无硬上限） | 1,000 | 2,015 | 62.74s | 31,878 events/s | 90.52 / 226.82 / 386.91ms |
| 100 | 1,000 | 2,002 | 54.36s | **36,792 events/s** | 97.01 / 179.03 / 199.43ms |
| 500 | 5,000 | 401 | 30.01s | **66,644 events/s** | 259.01 / 374.60 / 493.27ms |

每库500相对原实现提升约**2.09倍**，相对每库100提升约**1.81倍**；401个全局批次平均约
4,988 events/批，即约499 events/DB/事务，符合均匀路由预期。吞吐提高的代价是单批延迟增加，
因此默认500适合吞吐优先；低延迟部署可通过配置切换为100。下一层瓶颈是事务内部仍逐事件执行
`INSERT IGNORE + UPDATE`，继续提升需改为按`(db,table)`生成multi-row upsert/临时表join更新。
